...
export function HeroMockup() {
  return (
    <div className="mx-auto w-full max-w-[860px]">
      <div className="rounded-[26px] border border-ink/10 bg-white p-2 shadow-soft sm:p-3">
        <Image
          src={screenshot}
      ...
      <div className="mx-auto mt-5 grid max-w-3xl gap-3 sm:grid-cols-3">
        {site.proof.map((item) => {
          ...
          return (
            <div
              key={item.label}
              className="flex min-h-16 items-center gap-3 rounded-lg border border-ink/10 bg-white px-4 py-3 text-left shadow-sm"
            >
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-tomato/10 text-tomato">
                <Icon className="h-5 w-5" aria-hidden />
              </span>
              <span className="text-sm font-extrabold leading-5 text-ink">{item.label}</span>
            </div>
          );
        })}
      </div>
